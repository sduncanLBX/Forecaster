...
PipelineContents = [("scaler", StandardScaler()), ("regressor", LinearRegression(fit_intercept=True))]
...
process_note = "20-24-11-23 - only 1-variable"
...
def process_variable_set():
    global df, mseval, maxresults, modelid, target_mape, ResultsSavedCount, shifts, rsquared, X, min_months_since_1900, max_months_since_1900, num_samples, coeffients, writeflag, depvar_detrend, indepvar_detrend_scaled, ShiftedPredictionData, test_rsquared, test_mse, test_mae, test_mape, predict_mae, predict_mae, predict_mse, predict_mape, predict_rsquared
    ...
    for shifts in shift_combinations:
        ...
        if test_rsquared >= 0 or True:  # or (mseval < MSETarget)
            if SavePredictions:
        ...
        print(
            "ModelID:{:7d}, Depth:{:1d} Saved: {:6d}, Dep Var: {:>25.25}, Indep Var: {:<75.75}, Target MAPE: {:1.3f}\r".format(
                modelid,
                combos,
                ResultsSavedCount,
                DependentVar,
