...
def process_variable_set():
    ...
    for shifts in shift_combinations:
        ...
        if (test_rsquared >= 0.50) and (test_mape <= 0.30):  # or (mseval < MSETarget)
            if SavePredictions:
        ...
        print(
            "ModelID:{:7d}, Depth:{:1d} Saved: {:6d}, Dep Var: {:>25.25}, Indep Var: {:<75.75}, MAPE: {:1.3f}, R^2: {:1.3f}\r".format(
                modelid,
                combos,
                ResultsSavedCount,
                DependentVar,
